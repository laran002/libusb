...
/* One JVM per process -- deliberately not per-context state, and not
 * routed through default_context_options: see op_set_option()'s
...
#ifdef __ANDROID__
	if (javaVm != NULL) {
		r = android_jni(javaVm, &cpriv->android_jni);
		if (r != LIBUSB_SUCCESS)
			return r;
...
{
#ifdef __ANDROID__
	/* ctx may be NULL here (see the ctx == NULL carve-out in core.c's
	 * libusb_set_option()): the JavaVM* is process-wide, so there's
	 * nothing per-context to touch yet. */
