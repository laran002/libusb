...
{
#ifdef __ANDROID__
	/* ctx is always a real, resolved context here (libusb_set_option()
	 * in core.c never dispatches to the backend with ctx == NULL) -- but
	 * the JavaVM* itself is process-wide, not per-context, so it's cached
	 * in the global default_context_options rather than in cpriv. */
	if (option == LIBUSB_OPTION_ANDROID_JNIENV || option == LIBUSB_OPTION_ANDROID_JAVAVM) {
